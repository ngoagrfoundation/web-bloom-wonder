...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $stmt = $pdo->query("SELECT * FROM sponsors ORDER BY sort_order ASC, id ASC");
    echo json_encode(['data' => $stmt->fetchAll()]);

} elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = getJsonBody();
    $stmt = $pdo->prepare("INSERT INTO sponsors (name, logo, website_url, sort_order, is_published) VALUES (?, ?, ?, ?, ?)");
    ...
    if ($fields) { $params[] = $data['id']; $pdo->prepare("UPDATE sponsors SET " . implode(', ', $fields) . " WHERE id = ?")->execute($params); }
