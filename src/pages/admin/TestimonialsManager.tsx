...
const TestimonialsManager = () => {
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ quote: "", name: "", role: "", photo: "", is_published: 1, sort_order: items.length });
    setDialogOpen(true);
  };

  const openEdit = (t: Testimonial) => {
    setEditing(t);
    setForm({ quote: t.quote, name: t.name, role: t.role, photo: t.photo, is_published: t.is_published, sort_order: t.sort_order });
    setDialogOpen(true);
  };
  ...
  const remove = async (id: number) => {
    ...
    try {
      await adminFetch(`${API_BASE_URL}/admin/testimonials.php?id=${id}`, { method: "DELETE" });
