...
const PartnersManager = () => {
  ...
  const load = async () => {
    setLoading(true);
    try { const res = await adminFetch(`${API_BASE_URL}/admin/partners.php`); setItems(res.data || []); }
  ...
  const openEdit = (p: Partner) => { setEditing(p); setForm({ name: p.name, logo: p.logo, website_url: p.website_url, sort_order: p.sort_order, is_published: p.is_published }); setLogoFile(null); setDialogOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      let logo = form.logo;
      if (logoFile) {
        const result = await uploadImage(logoFile, "partners");
        if (result.path) logo = result.path;
      }
      const payload = { ...form, logo };
      if (editing) {
        await adminFetch(`${API_BASE_URL}/admin/partners.php`, { method: "PUT", body: JSON.stringify({ id: editing.id, ...payload }) });
        toast.success("Updated");
      } else {
        await adminFetch(`${API_BASE_URL}/admin/partners.php`, { method: "POST", body: JSON.stringify(payload) });
        toast.success("Added");
      }
  ...
  const remove = async (id: number) => {
    ...
    try { await adminFetch(`${API_BASE_URL}/admin/partners.php?id=${id}`, { method: "DELETE" }); toast.success("Deleted"); load(); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openNew} className="rounded-lg"><Plus className="w-4 h-4 mr-2" />Add Partner</Button>
      </div>
      ...
      {loading ? (
        ...
        <Card className="rounded-xl shadow-sm"><CardContent className="py-16 flex flex-col items-center text-muted-foreground gap-2"><Inbox className="h-8 w-8 text-muted-foreground/40" /><p className="text-sm">No partners yet</p></CardContent></Card>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {items.map((p) => (
            <Card key={p.id} className="rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
              <CardContent className="p-4 text-center">
                {p.logo ? <img src={p.logo} alt={p.name} className="h-16 mx-auto object-contain mb-3" /> : <div className="h-16 flex items-center justify-center text-muted-foreground text-2xl font-bold mb-3">{p.name[0]}</div>}
                <p className="font-medium text-sm truncate">{p.name}</p>
                {!p.is_published && <span className="text-xs text-muted-foreground">(Hidden)</span>}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <Button onClick={save} disabled={uploading} className="w-full">{uploading ? "Uploading..." : editing ? "Update" : "Add"} Partner</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
