...
const slides: SlideData[] = [
  ...
  {
    image: sanskritHeroImage,
    alt: "Free online Sanskrit classes with ancient manuscripts and diyas",
    title: "Free Online,",
    subtitle: "Sanskrit Classes",
    description: "Learn the ancient language of Sanskrit from the comfort of your home. Free online classes for all ages with experienced teachers.",
    link: "/programs/learning-sanskrit",
    hasRegister: true,
    registerType: "sanskrit",
  },
  {
    image: dentalHeroImage,
    alt: "Free dental treatment camp for community",
    title: "Free Dental,",
    subtitle: "Treatment Camp",
    description: "Get free dental check-ups and treatment at our community health camps. Quality dental care accessible to everyone.",
...
const HeroSection = () => {
  ...
  const handleSanskritSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!sanskritForm.name || !sanskritForm.mobile || !sanskritForm.batch) {
      toast.error("Please fill in all required fields");
      return;
    }
    if (!/^[6-9]\d{9}$/.test(sanskritForm.mobile)) {
    ...
    setTimeout(() => {
      ...
      setSanskritSubmitting(false);
      setShowSanskritForm(false);
    }, 1000);
  };

  const handleDentalSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!dentalForm.name || !dentalForm.mobile) {
      toast.error("Please fill in all required fields");
      return;
    }
    if (!/^[6-9]\d{9}$/.test(dentalForm.mobile)) {
      toast.error("Please enter a valid 10-digit mobile number");
      return;
    }
  ...
  return (
    <>
      <section id="home" className="relative h-[70vh] min-h-[500px] pt-20">
                  ...
                  <div className="max-w-2xl">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-6 md:mt-8">
                      ...
                      {slide.hasRegister && slide.registerType ? (
                        <button
                          onClick={() => handleRegisterClick(slide.registerType!)}
                        ...
                        <a href="#get-involved" className="border-2 border-white text-white hover:bg-white hover:text-foreground transition-all duration-200 px-6 py-3 rounded-lg font-medium text-center">
                          Get Involved
                        </a>
                      )}
                    </div>
        ...
        <button onClick={scrollPrev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-colors z-10" aria-label="Previous slide">
          <ChevronLeft size={24} />
        </button>
        <button onClick={scrollNext} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-colors z-10" aria-label="Next slide">
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${index === selectedIndex ? "bg-white w-8" : "bg-white/50 w-2 hover:bg-white/70"}`}
      ...
      {/* Sanskrit Registration Modal */}
      <Dialog open={showSanskritForm} onOpenChange={setShowSanskritForm}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-primary">Register for Free Sanskrit Classes</DialogTitle>
          </DialogHeader>
          <ScrollArea className="max-h-[70vh]">
            <form onSubmit={handleSanskritSubmit} className="space-y-4 p-1">
              <div className="space-y-2">
                <Label htmlFor="sanskrit-name">Name <span className="text-destructive">*</span></Label>
                <Input id="sanskrit-name" placeholder="Enter your full name" value={sanskritForm.name} onChange={(e) => setSanskritForm(p => ({ ...p, name: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="sanskrit-mobile">Mobile Number <span className="text-destructive">*</span></Label>
                <Input id="sanskrit-mobile" placeholder="9999999999" maxLength={10} value={sanskritForm.mobile} onChange={(e) => setSanskritForm(p => ({ ...p, mobile: e.target.value.replace(/\D/g, "") }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="sanskrit-address">Address</Label>
                <Input id="sanskrit-address" placeholder="Enter your address" value={sanskritForm.address} onChange={(e) => setSanskritForm(p => ({ ...p, address: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="sanskrit-age">Age</Label>
                <Input id="sanskrit-age" type="number" placeholder="Enter your age" min="5" max="100" value={sanskritForm.age} onChange={(e) => setSanskritForm(p => ({ ...p, age: e.target.value }))} />
              </div>
              <div className="space-y-2">
                ...
                <Select value={sanskritForm.batch} onValueChange={(val) => setSanskritForm(p => ({ ...p, batch: val }))}>
                  <SelectTrigger id="sanskrit-batch">
              ...
              <Button type="submit" className="w-full" disabled={sanskritSubmitting}>
                {sanskritSubmitting ? "Submitting..." : "Register Now"}
              </Button>
            </form>
          </ScrollArea>
        </DialogContent>
      ...
      {/* Dental Registration Modal */}
      <Dialog open={showDentalForm} onOpenChange={setShowDentalForm}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-primary">Register for Free Dental Treatment</DialogTitle>
          </DialogHeader>
          <ScrollArea className="max-h-[70vh]">
            <form onSubmit={handleDentalSubmit} className="space-y-4 p-1">
              <div className="space-y-2">
                <Label htmlFor="dental-name">Name <span className="text-destructive">*</span></Label>
                <Input id="dental-name" placeholder="Enter your full name" value={dentalForm.name} onChange={(e) => setDentalForm(p => ({ ...p, name: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="dental-mobile">Mobile Number <span className="text-destructive">*</span></Label>
                <Input id="dental-mobile" placeholder="9999999999" maxLength={10} value={dentalForm.mobile} onChange={(e) => setDentalForm(p => ({ ...p, mobile: e.target.value.replace(/\D/g, "") }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="dental-address">Address</Label>
                <Input id="dental-address" placeholder="Enter your address" value={dentalForm.address} onChange={(e) => setDentalForm(p => ({ ...p, address: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="dental-problem">Describe Your Dental Problem</Label>
                <Textarea id="dental-problem" placeholder="Briefly describe your dental issue..." rows={3} value={dentalForm.problem} onChange={(e) => setDentalForm(p => ({ ...p, problem: e.target.value }))} />
              </div>
              <Button type="submit" className="w-full" disabled={dentalSubmitting}>
                {dentalSubmitting ? "Submitting..." : "Register Now"}
              </Button>
            </form>
          </ScrollArea>
        </DialogContent>
